...
{
    Write-Host -ForegroundColor White "Begin Health Check: $server"
    ...
    if ($serverinfo -eq $null )
    {
        ...
        Write-Host -ForegroundColor RED The Server Is Not Exchange Server
    }
    elseif ( $serverinfo.IsEdgeServer )
    {
        Write-Host -ForegroundColor White "This EdgeServer"
    }
    else
    {
        ...
        if ( $ip -ne $null )
        {
            ...
            if ($OS -eq $null)
            {
                ...
                switch ($ping)
                {
                    $true {    $serversummary += "$server" }
                    default { $serversummary += "$server " }
                }
            }
            else
            {
